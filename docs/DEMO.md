...
4. Explain the aggregate MoA entropy `1.0`, Top-1 margin `3.37e-06` and neighboring-token variation `2.55e-06`:
   the colorful argmax regions amplify tiny cold-start differences and are not learned semantics.
5. Change sample and layer to show that the UI is driven by 192 archived evidence entries, not a hard-coded image.
6. Click **Copy evidence metadata** and **Export current PNG**. Close by stating that the raw NPZ, geometry,
   batch-equivalence results, config, log and SHA-256 manifest are preserved beside the demo.
